...
```{r Ropts, echo=FALSE}
options(width=200, datatable.showProgress=FALSE)
...
##job.dir <- "/scratch/th798/data.table-revdeps/2026-03-02"
...
sacct.dt <- slurm::sacct(sacct.arg)[order(task)]
some.sacct <- sacct.dt[, .(
  task,
  check.time=Elapsed,
...
## Did data.table master build succeed?

If there is an error shown here, the build must be modified (typically build-time dependencies like litedown must be installed).

```{r}
build_lines <- readLines("build_dt.out")
cat(build_lines, sep="\n")
```
