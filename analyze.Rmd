...
not.avail.pattern <- list(
  ...
  after.colon='(?:.*\n)+?',
  "[*|\n]")
nc::capture_all_str(test.input, not.avail.pattern)
not.avail.parsed <- if(length(not.avail.vec)){
  ...
  not.avail.deps$after.colon
  not.avail.deps[1]
  unique(not.avail.deps)[, .(deps=paste(nc::capture_all_str(
    after.colon,
    "'",
    dep=".*?",
    "'")$dep, collapse=", ")),
  ...
  data.table(Package=character(), type=character(), deps=character())
}
...
available during a check, but were not. Ideally this table should be
empty, so if there are entries here, they should be reported to the
