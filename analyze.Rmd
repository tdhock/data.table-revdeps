...
mytab <- function(DT){
  ...
  for(int.col in c("sig.diffs","no.log","config.fail","dl.fail")){
    int.value <- copied[[int.col]]
    chr.value <- ifelse(int.value==0, "", paste(int.value))
    set(copied, j=int.col, value=chr.value)
  }
