...
## Read R versions, which should have been written by params.R.
R.versions <- sapply(c("release","devel"), function(v.lo){
  readLines(paste0("R_",toupper(v.lo)), warn=FALSE)
})
## Read all diffs.
sig.cols <- c(
  "first.bad.commit",
...
sig.diff.dt <- if(length(diff.file.vec)){
  ...
  empty.dt.args <- list()
  empty.dt.args[sig.cols] <- NA_character_
  do.call(data.table, empty.dt.args)
}
...
deps.sacct[
  log.pkg,
  log.disk.KB := file.size(log.file.vec)/1024,
  on="Package"]
