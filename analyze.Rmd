...
## TODO this will not work if there are no issues:
R.versions <- unique(sig.diff.all[["Rversion"]])
pvers("R", R.versions)
...
not.avail.parsed <- if(length(not.avail.vec)){
  ...
  dep.dt[, .SD[1], by=dep][, .(Package, type, dep)]
}else{
  data.table(Package=character(), type=character(), dep=character())
}
...
available during a check, but were not. Each dependent package is only
reported once (only the first log with that dependent package missing
is shown). Ideally this table should be empty, so if there are entries
here, they should be reported to the maintainer of this revdep check
system via [this issue
...
* `r JOBID` for `run_one.sh`, several tasks, one for each revdep,
