...
- create two crontab entries 
  - midnight (=1 0 * * * ~/bin/rsync-CRAN-src-contrib.sh=), run
    [[file:rsync-CRAN-src-contrib.sh]] on data transfer node (dtn2), which
    downloads all CRAN source packages.
  - 1am (=1 1 * * * ~/bin/sbatch_params.sh=), run [[file:sbatch_params.sh][sbatch_params.sh]] on
    login node (wind), which will ask the cluster to run =params.sh=,
